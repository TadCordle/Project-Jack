...
{
    class BotPlayerOld : StickFigure
    {
        ...
        public BotPlayerOld(World world, Vector2 position, Category collisionCat, float scale, float limbStrength, float limbDefense, bool evilSkin, Color color, PlayerIndex player, StickFigure[] dudes)
        ...
        {
            return new BotPlayerOld(world, startPosition, collisionCat, scale, limbStrength, limbDefense, EvilSkin, color, player, this.ListStickFigures);
        }
        ...
        private void ResetTarget()
        {
            ...
            foreach (StickFigure s in ListStickFigures)
            {
                ...
                {
                    if ((this.target != null) && (s is BotPlayerOld) && (float)random.NextDouble() < 0.25f)
                    {
